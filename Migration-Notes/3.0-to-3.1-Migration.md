...
2. GET: /cart/checkout/payments in `CheckoutEndpoint.java`
...
3. POST: /cart/checkout/payment in `CheckoutEndpoint.java`
...
4. DELETE: /cart/checkout/payment in `CheckoutEndpoint.java`
...
5. POST: /cart/checkout in `CheckoutEndpoint.java`
    ...
    > Note: this was modified to no longer require payments in the checkout invocation. Payments should be added to an order by an earlier invcation to /cart/checkout/payment

### Deleted methods
1. /cart/checkout/payment/response
...
3. The ID field in list grids are no longer shown by default. You can re-enable this column with a `showIds=true` query parameter (e.g. /admin/product?showIds=true)
